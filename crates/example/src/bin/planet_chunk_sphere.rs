    ...
    for face in 0..6 {
        for chunk_u in 0..CHUNKS_PER_FACE_EDGE {
            for chunk_v in 0..CHUNKS_PER_FACE_EDGE {
                ...
                // Position chunks deeper to capture more terrain variation
                // Terrain varies ±20m, chunks are 8m thick
                // Position so chunk top is at nominal surface, extending down to 492m
                let chunk_thickness = CHUNK_SIZE as f32 * VOXEL_SIZE; // 8m
...
fn generate_planet_chunk(face: usize, chunk_u: usize, chunk_v: usize) -> Chunk {
    ...
    // Fill chunk with voxels based on terrain height
