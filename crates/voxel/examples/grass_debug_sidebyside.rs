//! Grass Debug: 2 voxels side-by-side (2×1×1)
//!
//! Tests horizontal adjacency:
//! - Internal face between voxels should be culled
//! - External faces should show correct textures
//! - Greedy meshing should merge the top into single 2×1 quad
//!
...
fn setup(
    ...
    println!("\n=== Grass Debug: Side-by-Side (2×1×1) ===");
    println!("Testing 2 grass voxels horizontally adjacent");
    println!("\nControls:");
    ...
    println!("\nExpected:");
    println!("  - Top: 2×1 merged green grass quad");
    println!("  - Bottom: 2×1 merged brown dirt quad");
    println!("  - Z+ and Z-: 2×1 merged grass-side quads");
    println!("  - X+ and X-: 1×1 grass-side quads (ends)");
    ...
    if let Ok(mut transform) = camera_query.single_mut() {
        ...
        if keyboard.just_pressed(KeyCode::Digit1) {
            *transform = Transform::from_xyz(distance + 1.0, 0.5, 0.5)
            ...
            println!("\n[1] X+ (right end)");
            println!("    Expected: 1×1 grass-side texture");
            ...
            *transform = Transform::from_xyz(1.0, distance + 0.5, 0.5)
            ...
            println!("\n[3] Y+ (top)");
            println!("    Expected: 2×1 merged green grass quad");
        } else if keyboard.just_pressed(KeyCode::Digit4) {
            *transform = Transform::from_xyz(1.0, -distance + 0.5, 0.5)
                .looking_at(center, Vec3::Z);
            println!("\n[4] Y- (bottom)");
            println!("    Expected: 2×1 merged brown dirt quad");
        } else if keyboard.just_pressed(KeyCode::Digit5) {
            *transform = Transform::from_xyz(1.0, 0.5, distance + 0.5)
                .looking_at(center, Vec3::Y);
            println!("\n[5] Z+ (front)");
            println!("    Expected: 2×1 merged grass-side quad");
